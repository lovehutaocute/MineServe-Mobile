# JavaMC GO

<div align="center">

# ✅ 可以正常使用

**在安卓手机上直接运行 Minecraft Java 版服务端——无需 root。**

[English](./README.md) · [架构文档](./ARCHITECTURE.md)

</div>

JavaMC GO 是一款 Android 原生应用，可在手机上直接运行 Minecraft Java 版服务端，支持 **9 类服务端核心**、插件与模组管理、内置内网穿透、免 Root 数据访问、自动备份与后台保活。

## ✨ 特性

- **无需 root**：内置 Termux bootstrap rootfs + JDK，全部在应用内运行
- **9 类服务端核心**：Paper、Purpur、Fabric、Forge、NeoForge、Quilt、Vanilla、Velocity、BungeeCord（NeoForge/Quilt 自动执行 installer）
- **插件管理**：扫描 plugins/ 目录，启用/禁用、删除、本地上传、URL 安装
- **模组管理**：扫描 mods/ 目录（Fabric/Forge），`.jar.disabled` 启停、上传
- **Modrinth 对接**：搜索模组（多加载器筛选 + 下载量/相关性/最新排序）、一键安装、模组图标展示
- **玩家管理**：在线玩家列表（踢出/切换模式/OP）、进服离服历史（精确到秒）、OP/白名单/封禁列表
- **内网穿透**：frp（粘贴完整 `frpc.toml`）+ bore（纯 Kotlin，无需下载二进制）
- **免 Root 访问 data 目录**：内置 MTDataFilesProvider，MT 管理器可直接读写应用私有数据（含图文教程）
- **备份还原**：主世界/地狱/末地三维度快照，创建/列表/还原/删除/SAF 导出
- **彩色 MC 终端**：日志分级着色（ERROR 红/WARN 黄/隧道蓝/崩溃橙），自动滚动，快捷指令，一键复制
- **概览地址卡片**：局域网 + 公网穿透地址带复制按钮，一键打开 MC 终端
- **后台保活**：前台服务 + 开机自启 + WorkManager 周期保活
- **设备与网络监控**：内存/存储/电池 + 实时网络流量与速度
- **完整 server.properties 编辑**：73 个参数基础/高级分组，底部导航「配置」tab 直达
- **Material 3 UI**：Jetpack Compose 构建，沉浸式全屏，9-tab 底部导航

## 📋 系统要求

- Android 8.0 (API 26) 及以上
- arm64-v8a 架构（正式版仅构建 arm64）
- 建议至少 4GB RAM（运行 MC 服务端）
- 需要网络连接（首次初始化下载 Termux 环境和 JDK）

## 📥 下载安装

### 方式一：直接下载 APK

从 [Releases 页面](../../releases) 下载：

- `app-arm64-v8a-release.apk` — 绝大多数手机


### 方式二：自行编译

```bash
git clone <repo-url>
cd MCServerManager
./gradlew.bat assembleRelease
```

生成的 APK 在 `app/build/outputs/apk/release/`。

## 🚀 快速开始

1. **初始化环境**：首次打开应用，点击「初始化」，自动下载 Termux rootfs + JDK 25
2. **下载核心**：切到「下载」Tab，选择核心类型（推荐 Paper）与版本
3. **启动服务器**：首次启动需下载运行文件，请耐心等待
4. **加入游戏**：通过概览页局域网地址连接，或使用内置内网穿透公网访问

## 🧩 支持的核心

| 核心 | 插件 | 模组 | 说明 |
|---|---|---|---|
| Paper | ✅ | ❌ | 高性能优化核心，生产推荐 |
| Purpur | ✅ | ❌ | Paper 分支，更多特性开关 |
| Fabric | ❌ | ✅ | 轻量级模组加载器 |
| Forge | ❌ | ✅ | 老牌模组加载器 |
| NeoForge | ❌ | ✅ | Forge 继任者（installer 流程） |
| Quilt | ❌ | ✅ | Fabric 分支（installer 流程） |
| Vanilla | ❌ | ❌ | 官方原版 |
| Velocity | ❌ | ❌ | 代理端 |
| BungeeCord | ❌ | ❌ | 代理端 |

## 📖 功能详解

### 🖥️ 服务器控制与 MC 终端
- 一键启停，启动状态（运行时长 + TPS/在线/内存统计），崩溃自动重启
- **彩色日志**：ERROR 红 / WARN 黄 / `[tunnel]` 蓝 / `[crash]` 橙 / `[bootstrap]` 绿
- **自动滚动**：仅在位于底部附近时跟随新日志，上翻不被打断
- **快捷指令**：一键 `/list` `/tps` `/say` `/kick` `/help`，支持一键复制全部日志
- 概览页「打开 MC 终端」按钮直达日志页

### 📊 概览地址卡片
- 局域网地址（`127.0.0.1:port` / `192.168.x.x:port`）与内网穿透公网地址，各带复制按钮
- 服务器核心/插件/玩家状态一览

### 🌐 内网穿透
- **frp**：粘贴完整 `frpc.toml` 配置文本（服务端地址、token、代理规则一次配好）；frpc 缺失时自动下载（含多个国内镜像，并行尝试）
- **bore**：纯 Kotlin 客户端，无需下载二进制，连接任意 `bore server` VPS 即可
- 公网地址同步显示在概览卡片与网络页，均带复制按钮

### 📁 文件管理（免 Root）
- 浏览服务器目录、上传、新建文件夹、删除、导出（单文件/文件夹 zip/整服 zip，SAF 存储）
- **向上 / 刷新**按钮快速导航
- **MT 管理器集成**：内置 `MTDataFilesProvider`，在 MT 管理器中「添加本地存储 → 选择本应用」即可读写 `data` 目录；应用内提供下载链接 + 分步图文教程（进入 `data → files → home → home → servers` 查看服务器文件）
- 常见服务器文件/文件夹自动标注说明

### 👥 玩家管理
- 在线玩家列表（复制 / 踢出 / 切换游戏模式 / OP）
- 进服离服历史（精确到秒，重启保留）
- OP / 白名单 / 封禁列表（含限时封禁、OP 等级、搜索与详情）

### 🔌 插件与模组
- 插件：扫描 plugins/ 目录 + plugin.yml 元数据，启停/删除（含数据目录清理）、本地上传、URL 安装、资源站目录
- 模组：mods/ 目录扫描（Fabric/Forge），`.jar.disabled` 启停、删除、上传
- **Modrinth 对接**：搜索（多加载器筛选、下载量/相关性/最新排序）、图标展示、一键安装（自动匹配 MC 版本，不兼容核心给出警告）

### 💾 备份与还原
- 主世界/地狱/末地三维度快照（zip）
- 创建（自动 `save-all`）、列表、还原（自动停服并备份当前世界）、删除、SAF 导出
- 旧快照自动清理

### ⚙️ 配置
- 完整 `server.properties` 编辑器：73 个参数，基础/高级分组，数据驱动渲染，未收录参数兜底编辑
- JVM 堆内存限制、下载镜像、APT 镜像、自动重启开关

### 🛡️ 后台保活

纯 Android 方案（无需 NDK）：
- 前台服务 START_STICKY + 常驻通知
- 开机自启动（BOOT_COMPLETED 广播）
- WorkManager 每 15 分钟周期保活
- 专属保活页面：各开关独立配置 + 详细功能说明

> 提示：Android 系统对后台进程有严格限制，保活效果受系统版本与厂商策略影响；请勿在系统设置中限制本应用后台运行，并授予自启动权限。

## 📄 许可证

详见仓库 LICENSE 文件。
